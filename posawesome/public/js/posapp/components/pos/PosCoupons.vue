    ...
    >
      <v-card-title>
        <v-row no-gutters align="center" justify="center">
          <v-col cols="6">
            <span class="text-h6 text-primary">{{ $t("Coupons") }}</span>
          </v-col>
          <v-col cols="4">
            <v-text-field
              density="compact"
...
export default {
    ...
    add_coupon(new_coupon) {
      if (!this.customer || !new_coupon) {
        this.eventBus.emit("show_message", {
          text: this.$t("Select a customer to use coupon"),
          color: "error",
        });
        return;
      }
      ...
      if (exist) {
        this.eventBus.emit("show_message", {
          text: this.$t("This coupon already used!"),
          color: "error",
        });
        return;
      }
  ...
  created() {
      this.items_headers = [
        { title: this.$t("Coupon"), value: "coupon_code", align: "start" },
        { title: this.$t("Type"), value: "type", align: "start" },
        { title: this.$t("Offer"), value: "pos_offer", align: "start" },
        { title: this.$t("Applied"), value: "applied", align: "start" }
      ];
    ...
    this.$nextTick(() => {
      ...
      setTimeout(() => {
        if (this.customer && this.pos_profile?.company) {
